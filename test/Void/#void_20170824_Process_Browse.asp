...
select case request("cat")
       ...
       case 1
        ...
        case 2
         ttl = request.cookies("fmea.PjKey")
         sql="SELECT * FROM v_Fmea_Item where  RPN > 2  and PjID = '"&Request.cookies("fmea.PjID")&"' ORDER BY SubPjName,PageNo,WorkQueue" 
       ...
       '--全系列搜索 
        ...
        case 4
         ttl = request.cookies("fmea.PjKey")
         sql="SELECT * FROM v_Fmea_Item where PartsOK is not null and PjID = '"&Request.cookies("fmea.PjID")&"' ORDER BY SubPjName,PageNo,WorkQueue"
        ...
        case 6
         ttl = request.cookies("fmea.PjKey")
         sql="SELECT * FROM v_Fmea_Item where  RPN ="&request("Sc")&" and PjID = '"&Request.cookies("fmea.PjID")&"' ORDER BY SubPjName,PageNo,WorkQueue" 
...
while not rs.eof and n<= rs.pagesize
    ...
    if vRPN > 2 Then
       ...
       response.write"<div><a href=../history/His_Process_Browse.asp?ItmID="&ItmID&">修订履历</a></div>"
       end if
       if isnull(rs("ItmID_fb")) then
       else
         response.write"<div><a href= 'javascript:void(0);'  onClick=about('../His_Question/FBRead.asp?cat=3&ItmID="&ItmID&"',600,600)>过去问题</a></div>"
       end if
       response.write"</td>"       
    else
       ...
       if isnull(rs("ItmID_fb")) then
         ...
         response.write"<div><a href= #  onClick=about('../His_Question/FBRead.asp?cat=3&ItmID="&ItmID&"',600,600)>过去问题</a></div>"
       end if
       response.write"</td>"
    end if 
    ...
    if request.cookies("fmea.power") > "1" then 
       ...
       if vRPN > 2 Then
         response.write"<div><a href= #  onClick=about('Advice_Add.asp?cat=1&ItmID="&ItmID&"',800,250)>措施填入</a></div>"
       end if
